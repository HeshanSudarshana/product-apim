...
public class SemanticToolFilteringTestCase extends APIMIntegrationBaseTest {
    ...
    private static final String GUARDRAIL_DEPLOYMENT_TOML = "guardrail" + File.separator + "deployment.toml";
    ...
    public void setEnvironment() throws Exception {
        ...
        serverConfigurationManager.applyConfigurationWithoutRestart(resolveGuardrailDeploymentToml());
    ...
    private File resolveGuardrailDeploymentToml() throws IOException {
        File resourceLocationCandidate = new File(getAMResourceLocation(), GUARDRAIL_DEPLOYMENT_TOML);
        if (resourceLocationCandidate.isFile()) {
            return resourceLocationCandidate.getCanonicalFile();
        }

        String basedir = System.getProperty("basedir");
        if (basedir != null) {
            File basedirCandidate = new File(basedir, "src" + File.separator + "test" + File.separator
                    + "resources" + File.separator + GUARDRAIL_DEPLOYMENT_TOML);
            if (basedirCandidate.isFile()) {
                return basedirCandidate.getCanonicalFile();
            }
        }

        File relativeCandidate = new File("src" + File.separator + "test" + File.separator
                + "resources" + File.separator + GUARDRAIL_DEPLOYMENT_TOML);
        if (relativeCandidate.isFile()) {
            return relativeCandidate.getCanonicalFile();
        }

        throw new IOException("Guardrail deployment.toml not found");
    }
